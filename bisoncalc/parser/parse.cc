...
PI__ const s_productionInfo[] = 
{
     ...
     {327, 1}, // 1: start ->  command_list
     {328, 3}, // 2: command_list (COMMAND_SEPARATOR) ->  command_list command COMMAND_SEPARATOR
     {328, 2}, // 3: command_list (COMMAND_SEPARATOR) ->  command COMMAND_SEPARATOR
     {329, 4}, // 4: command (IDENTIFIER) ->  IDENTIFIER OPARENTHESIS arguments CPARENTHESIS
     {329, 3}, // 5: command (IDENTIFIER) ->  IDENTIFIER OPERATOR_LET_BE expression
     {329, 4}, // 6: command (KEYWORD_LOCAL) ->  KEYWORD_LOCAL IDENTIFIER OPERATOR_LET_BE expression
     {329, 3}, // 7: command (OCURLY) ->  OCURLY command_list CCURLY
     {329, 7}, // 8: command (KEYWORD_FOR) ->  KEYWORD_FOR IDENTIFIER KEYWORD_IN expression_list KEYWORD_DO command_list KEYWORD_DONE
     {329, 9}, // 9: command (KEYWORD_FOR) ->  KEYWORD_FOR command COMMAND_SEPARATOR expression COMMAND_SEPARATOR command KEYWORD_DO command_list KEYWORD_DONE
     {329, 5}, // 10: command (KEYWORD_WHILE) ->  KEYWORD_WHILE expression KEYWORD_DO command_list KEYWORD_DONE
     {329, 5}, // 11: command (KEYWORD_UNTIL) ->  KEYWORD_UNTIL expression KEYWORD_DO command_list KEYWORD_DONE
     {329, 9}, // 12: command (KEYWORD_CASE) ->  KEYWORD_CASE expression KEYWORD_DO caseparts KEYWORD_DEFAULT CPARENTHESIS command_list CASE_DELIMITER KEYWORD_ESAC
     {329, 6}, // 13: command (KEYWORD_IF) ->  KEYWORD_IF expression KEYWORD_THEN command_list else_part KEYWORD_FI
     {330, 3}, // 14: expression (OPARENTHESIS) ->  OPARENTHESIS expression CPARENTHESIS
     {330, 2}, // 15: expression (NEGATE) ->  NEGATE expression
     {330, 3}, // 16: expression (OPERATOR_AND) ->  expression OPERATOR_AND expression
     {330, 3}, // 17: expression (OPERATOR_OR) ->  expression OPERATOR_OR expression
     {330, 3}, // 18: expression (OPERATOR_EQUAL) ->  expression OPERATOR_EQUAL expression
     {330, 3}, // 19: expression (OPERATOR_NOT_EQUAL) ->  expression OPERATOR_NOT_EQUAL expression
     {330, 3}, // 20: expression (OPERATOR_LESS_EQUAL) ->  expression OPERATOR_LESS_EQUAL expression
     {330, 3}, // 21: expression (OPERATOR_GREATER_EQUAL) ->  expression OPERATOR_GREATER_EQUAL expression
     {330, 3}, // 22: expression (OPERATOR_LESS_THAN) ->  expression OPERATOR_LESS_THAN expression
     {330, 3}, // 23: expression (OPERATOR_GREATER_THAN) ->  expression OPERATOR_GREATER_THAN expression
     {330, 3}, // 24: expression (OPERATOR_PLUS) ->  expression OPERATOR_PLUS expression
     {330, 3}, // 25: expression (OPERATOR_MINUS) ->  expression OPERATOR_MINUS expression
     {330, 3}, // 26: expression (OPERATOR_MULTIPLICATION) ->  expression OPERATOR_MULTIPLICATION expression
     {330, 3}, // 27: expression (OPERATOR_DIVISION) ->  expression OPERATOR_DIVISION expression
     {330, 3}, // 28: expression (OPERATOR_MODULO) ->  expression OPERATOR_MODULO expression
     {330, 3}, // 29: expression (OPERATOR_POWER) ->  expression OPERATOR_POWER expression
     {330, 2}, // 30: expression (UNARY_MINUS) ->  OPERATOR_MINUS expression
     {330, 4}, // 31: expression (IDENTIFIER) ->  IDENTIFIER OSBRACKET expression CSBRACKET
     {330, 1}, // 32: expression (INTEGER) ->  INTEGER
     {330, 1}, // 33: expression (DOUBLE) ->  DOUBLE
     {330, 1}, // 34: expression (SINGLEQUOTE) ->  SINGLEQUOTE
     {330, 1}, // 35: expression (DOUBLEQUOTE) ->  DOUBLEQUOTE
     {330, 1}, // 36: expression (HEXADECIMAL) ->  HEXADECIMAL
     {330, 1}, // 37: expression ->  input_output
     {330, 1}, // 38: expression (IDENTIFIER) ->  IDENTIFIER
     {330, 3}, // 39: expression (OSBRACKET) ->  OSBRACKET arraypart CSBRACKET
     {330, 1}, // 40: expression (CONST_TRUE) ->  CONST_TRUE
     {330, 1}, // 41: expression (CONST_FALSE) ->  CONST_FALSE
     {330, 1}, // 42: expression (CONST_PI) ->  CONST_PI
     {330, 4}, // 43: expression (COMMAND_RETURNER) ->  COMMAND_RETURNER OCURLY command CCURLY
     {331, 3}, // 44: arraypart (ELEMENT_SEPARATOR) ->  arraypart ELEMENT_SEPARATOR expression
     {331, 1}, // 45: arraypart ->  expression
     {331, 0}, // 46: arraypart ->  <empty>
     {332, 3}, // 47: arguments (ELEMENT_SEPARATOR) ->  arguments ELEMENT_SEPARATOR expression
     {332, 1}, // 48: arguments ->  expression
     {332, 0}, // 49: arguments ->  <empty>
     {333, 2}, // 50: input_output (OUTPUT_CREATION) ->  OUTPUT_CREATION IDENTIFIER
     {333, 2}, // 51: input_output (INPUT_REDIRECTION) ->  INPUT_REDIRECTION IDENTIFIER
     {333, 2}, // 52: input_output (INPUT_FROM_A_HERE_DOCUMENT) ->  INPUT_FROM_A_HERE_DOCUMENT IDENTIFIER
     {333, 2}, // 53: input_output (OUTPUT_APPEND) ->  OUTPUT_APPEND IDENTIFIER
     {334, 2}, // 54: caseparts ->  caseparts case_part
     {334, 1}, // 55: caseparts ->  case_part
     {335, 4}, // 56: case_part (CPARENTHESIS) ->  expression CPARENTHESIS command_list CASE_DELIMITER
     {336, 2}, // 57: else_part (KEYWORD_ELSE) ->  KEYWORD_ELSE command_list
     {336, 0}, // 58: else_part ->  <empty>
     {337, 3}, // 59: expression_list (ELEMENT_SEPARATOR) ->  expression_list ELEMENT_SEPARATOR expression
     {337, 1}, // 60: expression_list ->  expression
     {338, 1}, // 61: start_$ ->  start
};
...
SR__ s_20[] =
{
    ...
    { {       301}, { 52} }, // OPERATOR_AND           
    { {       302}, { 53} }, // OPERATOR_OR            
    { {       303}, { 54} }, // OPERATOR_EQUAL         
    { {       308}, { 55} }, // OPERATOR_NOT_EQUAL     
    { {       307}, { 56} }, // OPERATOR_LESS_EQUAL    
    { {       306}, { 57} }, // OPERATOR_GREATER_EQUAL 
    { {       305}, { 58} }, // OPERATOR_LESS_THAN     
    { {       304}, { 59} }, // OPERATOR_GREATER_THAN  
    { {       309}, { 60} }, // OPERATOR_PLUS          
    { {       310}, { 61} }, // OPERATOR_MINUS         
    { {       311}, { 62} }, // OPERATOR_MULTIPLICATION
    { {       312}, { 63} }, // OPERATOR_DIVISION      
    { {       313}, { 64} }, // OPERATOR_MODULO        
    { {       314}, { 65} }, // OPERATOR_POWER         
    { {         0}, {  0} },                           
};
